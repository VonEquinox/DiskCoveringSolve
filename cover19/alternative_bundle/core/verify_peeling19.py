...
def check_constants():
    assert S.check_geometry() and S.N==G.N==19 and S.B==12 and G.V==3*S.N-2
    assert (G.A,G.C,G.C6,G.D)==(894563,1871672,3128331,10000000)
    assert S.T==F(1,13) and 0<S.T<G.RU**2<F(1,4)
    gaps=[F(G.A,G.D),F(G.C,G.D),F(G.C6,G.D)]
    assert 0<gaps[0]<gaps[1]<gaps[2]<F(1,2) and 11*gaps[0]<1
    margins=[]
    ...
    return {'radius_upper':str(G.RU),'turn_caps':list(map(str,gaps)),
            'sine_margins':margins,'minimum_boundary_cells':12,
            'geometry_sha256':sha(S.R/'geometry19.py')}

def bfs_code(B,faces):
    """Independent Python code: inspect all oriented roots, not a hash guess."""
    N=19
    assert 12<=B<N and len(faces)==2*N-B-2
...
def verify():
    ...
    rows=json.loads((ENUM/'survivors19.json').read_text())
    ...
    for B in range(12,19):
        ...
        for out in (primary,audit):
            assert out['N']==19 and out['B']==B and out['I']==19-B
        ...
        for index,masks in enumerate(representatives):
            assert all(type(m) is int and 0<m<(1<<19) and m.bit_count()==3 for m in masks)
            assert masks==sorted(set(masks))
            fs=[[i for i in range(19) if m>>i&1] for m in masks]
            row={'B':B,'I':19-B,'idx':index,'faces':fs}
        ...
        count=labelled(B,19-B)
        assert count%factorial(19-B)==0
        closed=2*factorial(2*B-3)*factorial(4*(19-B)+2*B-5)//(
            factorial(B-1)*factorial(B-3)*factorial(3*(19-B)+2*B-3))
        assert count==closed
        families[f'B{B}_I{19-B}']={
            ...
            'unlabelled_interior_rooted_count':count//factorial(19-B),
    ...
    out={'verified':True,'method':'Exhaustive root-face recursion with exact cuts BEFORE completion; independent DFS/Bellman and BFS/Floyd implementations.',
         ...
         'all_families':[12,13,14,15,16,17,18],'unresolved_combinatorial_branches':0,
         'survivors_sha256':sha(ENUM/'survivors19.json'),'seconds':time.time()-started}
    (REPORTS/'peeling19_verified.json').write_text(json.dumps(out,indent=2))
