...
These tests are additional checks, not the completeness proof of the n=19 run.
"""
from pathlib import Path
from math import factorial
import json, subprocess, tempfile, time, hashlib
from verify_peeling19 import labelled
if not __debug__:
...
def verify():
    ...
    cases += [(11,8),(12,9)]
    with tempfile.TemporaryDirectory(prefix='recursion19_',dir=ROOT/'_runtime') as tmp:
        for n,b in cases:
            outputs=[]
            for method in ('dfs','bfs'):
                path=Path(tmp)/f'{method}_{n}_{b}.json'
                args=[str(ROOT/'_runtime'/f'peel_{method}19'),str(n),str(b),str(path)]
    ...
    out={'verified':True,'tests':tests,'cases':len(tests),'pruning_disabled':True,
         'role':'Additional finite checks; n=19 completeness is the root-face induction plus freshly executed exhaustive searches.',
         'seconds':time.time()-start}
    (ROOT/'reports'/'recursion_tests19_verified.json').write_text(json.dumps(out,indent=2))
