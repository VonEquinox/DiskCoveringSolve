...
def rows_from_manifest(b,path):
    ...
    for idx,line in enumerate(path.read_text().splitlines()):
        ints=list(map(int,line.split()));assert len(ints)==1+36-b
        stab,*masks=ints;assert stab>0 and masks==sorted(set(masks))
        assert all(0<m<1<<19 and m.bit_count()==3 for m in masks)
        faces=sorted([[j for j in range(19) if m>>j&1] for m in masks])
        sig=G.signature(b,faces);assert sig is not None
        G.bounds(b,faces)
        rows.append({'B':b,'I':19-b,'idx':idx,'faces':faces,'signature':list(sig),'stabilizer':stab})
    return rows

def verify():
    (R/"enumeration19_verified.json").unlink(missing_ok=True)
    t=time.time();results=[];rows=[];regression=[]
    with tempfile.TemporaryDirectory(prefix='cover19_enum_') as td:
        tmp=Path(td);primary,audit=compile_tools(tmp)
        for b in range(12,19):
            prefix=tmp/f'B{b}';archive=E/f'B{b}_I{19-b}.txt';report=tmp/f'audit_{b}.json'
            log=run([primary,19,b,G.A,G.C,G.C6,prefix,1])
            data=json.loads(prefix.with_suffix('.json').read_text())
            assert (data['N'],data['B'],data['I'],data['A'],data['C4'],data['C6'],data['denominator'],data['pruning'],data['complete'])==(19,b,19-b,G.A,G.C,G.C6,G.D,True,True)
            assert prefix.with_suffix('.txt').read_bytes()==archive.read_bytes(),('regenerated manifest differs',b)
            log+=run([audit,19,b,G.A,G.C,G.C6,archive,report,1])
            other=json.loads(report.read_text());assert other['verified'] and (other['N'],other['B'],other['A'],other['C4'],other['C6'])==(19,b,G.A,G.C,G.C6)
            ...
            rows.extend(br)
            results.append({'B':b,'I':19-b,'primary':data,'independent':other,'manifest_sha256':sha(archive),'log_sha256':hashlib.sha256(log.encode()).hexdigest()})
            print('FRESH COMPLETE ENUMERATION + INDEPENDENT AUDIT',b,19-b,len(br),flush=True)
        ...
        for n in range(7,11):
            ...
            assert c['rooted_survivors']==a['rooted_survivors'] and c['surviving_orbits']==a['orbits']
            regression.append({'N':n,'B':b,'I':i,'labelled_count':lab,'rooted_count':a['rooted_survivors'],'orbits':a['orbits']})
    archived=json.loads((E/'metric19_residuals.json').read_text())
    assert rows==archived,'full residual rows do not equal regenerated surviving records'
    out={'verified':True,'N':19,'method':'exhaustive rooted peeling with safe necessary-condition pruning; independent queue/root/label/BFS/Floyd replay',
         ...
         'residual_sha256':sha(E/'metric19_residuals.json'),'seconds':time.time()-t}
    (R/'enumeration19_verified.json').write_text(json.dumps(out,indent=2))
