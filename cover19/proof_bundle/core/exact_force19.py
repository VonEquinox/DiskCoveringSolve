...
A=8946;C=18717;C6=31284;D=100000;RU=F('0.27735009811263')

def edges(B,faces):
 assert len(faces)==36-B
...
def tighten(M,lo,hi):
 ...
 q=lcm(D,*(v.denominator for v in l+h));fac=q//D
 ll=[v.numerator*(q//v.denominator) for v in l];hh=[v.numerator*(q//v.denominator) for v in h]
 nh=[min(hh[i]+M[i][j]*fac for i in range(B)) for j in range(B)]
 nl=[max(ll[i]-M[j][i]*fac for i in range(B)) for j in range(B)]
 if any(a>b for a,b in zip(nl,nh)):return None
 return [F(v,q) for v in nl[1:]],[F(v,q) for v in nh[1:]]
def ceilroot(n):
