...
int main(int argc,char**argv){
 ...
 string pre=argv[6];std::remove((pre+".json").c_str());std::remove((pre+".txt").c_str());
 N=stoi(argv[1]);B=stoi(argv[2]);I=N-B;FMAXOUT=2*N-B-2;CA=stoi(argv[3]);CC4=stoi(argv[4]);CC6=stoi(argv[5]);PRUNE=stoi(argv[7]);if(argc>8)limit=stoull(argv[8]);
