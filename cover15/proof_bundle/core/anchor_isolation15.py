...
GAMMA=300; LAMBDA=F(18,10000); NU=F(1,80)
BUFFER=F(1,10**9); EPS=F(2,Q); RADIUS=F(13,100)

def h(p):return hashlib.sha256(p.read_bytes()).hexdigest()
def prepare():
 c=json.loads((ROOT/'root15.json').read_text())
 ...
 A=[[0]*S.G for _ in range(S.C)];M=[[0]*S.G for _ in range(S.G)];degree=[0]*(1+S.G//2)
 ...
 assert sum(v*v for row in A for v in row)<121*Q*Q
 dA=300*EPS; dM=(2*max(degree)+1)*EPS
 dD=EPS/4 # |w^2-wbar^2| <= (|w|+|wbar|) EPS < EPS/4.
 assert 100*max(di)<Q*Q
 error=dM+GAMMA*(F(1,100)*(22+dA)*dA+dD*(11+dA)**2)
 assert error<BUFFER
...
def main():
 ...
 assert cert['root_sha256']==h(ROOT/'root15.json')
 assert cert['root_data_sha256']==h(ROOT/'system15.py')
 ...
 out={'verified':True,'theorem':'Any realization of the 61-rod candidate graph with all rods <= r_star, unit anchors, fixed q0=(1,0), and ||Q-Q_star||_F <=13/100, is the candidate itself.',
 'gamma':GAMMA,'lambda':str(LAMBDA),'nu_upper':str(NU),'anchor_Frobenius_radius':str(RADIUS),
 'per_anchor_angle_halfwidth':str(F(1,25)),
