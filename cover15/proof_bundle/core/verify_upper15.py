"""Exact coverage by 15 disks: 39 triangles and eleven circular caps."""
...
def verify():
 ...
 assert B==11 and N==15 and S.G==76 and S.M==61
 assert len(S.FACES)==2*N-B-2 and len(set(tuple(f) for f in S.FACES))==len(S.FACES)
 edges={frozenset(e) for e in S.EDGES}
 active_w={tuple(f):B+N+i for i,f in enumerate(S.ACTIVE)}
 equality_faces=0;slack=[]
 for face0 in S.FACES:
  face=tuple(face0)
  assert len(set(face))==3 and all(type(c)is int and 0<=c<N for c in face)
  if face in active_w:
   assert all(frozenset((B+c,active_w[face])) in edges for c in face);equality_faces+=1
  else:
   ...
   mar=min(tlo-I.norm2(I.vsub(p[B+i],wiv))[1] for i in face);assert mar>0,(face,mar)
   slack.append({'face':list(face),'witness':list(map(str,w)),'margin':str(mar)})
 assert equality_faces==13 and len(slack)==4
 aa=json.load(open(R/'anchor_angles15.json'));cent=list(map(F,aa['centers']));h=F(aa['halfwidth'])
 assert len(cent)==B-1 and 0<h<F(1,1000) and 0<cent[0]-h and cent[-1]+h<1
 ...
 directed=Counter();ec=Counter();links=defaultdict(list);minarea=None;oriented=[];vadj=defaultdict(set)
 for tri in triangles:
  ...
  for j in range(3):
   u,v=tri[j],tri[(j+1)%3];directed[u,v]+=1;ec[tuple(sorted((u,v)))]+=1;links[u].append((tri[(j+1)%3],tri[(j+2)%3]));vadj[u].add(v);vadj[v].add(u)
 assert set(links)==set(range(B+N)) and len(triangles)==39 and len(ec)==64 and B+N-len(ec)+len(triangles)==1
 seen={0};stack=[0]
 while stack:
  for j in vadj[stack.pop()]:
   if j not in seen:seen.add(j);stack.append(j)
 assert seen==set(range(B+N))
 ...
 out={'verified':True,'vertices':B+N,'edges':len(ec),'faces':len(triangles),'boundary_caps':B,'exact_equality_center_faces':equality_faces,'strict_center_faces':slack,'minimum_orientation':str(minarea),'minimum_arc_wedge_margin':str(minarc),'oriented_triangles':oriented,'seconds':time.time()-st}
 (R/'upper15_verified.json').write_text(json.dumps(out,indent=2));print('UPPER COVER VERIFIED',len(triangles),'triangles,',equality_faces,'equality faces; sec',time.time()-st,flush=True)
 return out
