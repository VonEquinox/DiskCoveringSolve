...
static constexpr int N=15, MAXF=18;
struct State {
...
void gen_groups(){
    iperms.clear(); array<int,5> p={0,1,2,3,4};
...
vector<Sig> boundary_signature(State const&s){
    ...
    for(int u=0;u<B;u++){
        ...
        for(int k=0;k<5;k++)q[2+k]=k<(int)ideg.size()?ideg[k]:-1;
        q[7]=ftype[u][0];q[8]=ftype[u][1];q[9]=ftype[u][2];q[10]=ftype[u][3-1];
        out[u]=q;
    }
    return out;
}
...
// Canonical form: boundary signature, then ordered internal degree/boundary-mask
// classes. Only permutations inside equal classes remain. This is exactly a
// canonical representative of D_B x S_I, not a heuristic graph hash.
pair<State,unsigned> canonical_with_stab(State const&s){
    ...
    array<array<uint8_t,N>,N> adj{};array<int,N> deg{};
    for(int k=0;k<FN;k++){auto t=dec3(s.f[k]);for(int a:t)for(int b:t)if(a!=b)adj[a][b]=1;}
    for(int a=0;a<N;a++)for(int b=0;b<N;b++)deg[a]+=adj[a][b];
    bool have=false;State best;unsigned stab=0;
    for(int bi:keep){
        auto bm=boundary_maps[bi];
        vector<pair<pair<int,int>,int>> keys;
        for(int v=B;v<N;v++){
            int mask=0;for(int u=0;u<B;u++)if(adj[v][u])mask|=1<<bm[u];
            keys.push_back({{deg[v],mask},v});
        }
        sort(keys.begin(),keys.end());
        vector<int> perm(I);for(int k=0;k<I;k++)perm[k]=keys[k].second;
        vector<pair<int,int>> blocks;
        for(int a=0;a<I;){int b=a+1;while(b<I&&keys[b].first==keys[a].first)b++;blocks.push_back({a,b});a=b;}
        bool more=true;
        while(more){
            auto m=bm;for(int k=0;k<I;k++)m[perm[k]]=B+k;
            State z=transform(s,m);
            if(!have||z<best){best=z;have=true;stab=1;}else if(z==best)stab++;
            more=false;
            for(auto [a,b]:blocks){if(next_permutation(perm.begin()+a,perm.begin()+b)){more=true;break;}}
        }
    }
    if(!have||!stab)throw runtime_error("canonical failure");
    return {best,stab};
}
State canonical(State const&s){return canonical_with_stab(s).first;}
...
uint64_t orbit_size(State const&s){auto z=canonical_with_stab(s);if(group_maps.size()%z.second)throw runtime_error("stab division");return group_maps.size()/z.second;}
...
int main(int argc,char**argv){
    ...
    if(B+I!=N||B<3||I<0||I>5)throw runtime_error("bad B,I");FN=B+2*I-2;gen_groups();
    auto t0=chrono::steady_clock::now();State s0=seed();string why;if(!validate(s0,&why))throw runtime_error("invalid seed "+why);
    unordered_set<State,HState> seen;seen.reserve(1000000);vector<State> reps;reps.reserve(1000000);queue<uint32_t> q;seen.insert(s0);reps.push_back(s0);q.push(0);
    uint64_t attempts=0;
    while(!q.empty()){
      ...
      if(id%100000==0){double sec=chrono::duration<double>(chrono::steady_clock::now()-t0).count();cerr<<"orbits "<<reps.size()<<" queue "<<q.size()<<" sec "<<sec<<"\n";}
    }
    {
    ...
    uint64_t sumorb=0,minorb=~0ULL,maxorb=0;array<uint64_t,2500> hist{};
