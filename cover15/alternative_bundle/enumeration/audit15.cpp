...
struct CheckedGraph {
 ...
 bool signature(vector<int>&masks,int A,int C,int D)const {
  masks.clear();
  for(int i=0;i<B;i++){
   unsigned reach=adj[i]|adj[(i+1)%B]|(1u<<i)|(1u<<((i+1)%B));
   for(int j=i+1;j<B;j++){
    if(!(reach&((1u<<j)|(1u<<((j+1)%B)))))continue;int k=j-i;if(min(k,B-k)<=2)continue;
    bool f=k*A<D-C,b=(B-k)*A<D-C;if(f&&b)return false;
    int mask=((1u<<j)-1)^((1u<<i)-1);
    if(f)masks.push_back(mask);else if(b)masks.push_back(((1u<<B)-1)^mask);
   }
  }
...
int main(int argc,char**argv){try{
 if(argc!=5)throw runtime_error("usage: audit15 masks farkas expected_labelled output_prefix");
 ifstream in(argv[1],ios::binary);uint32_t hdr[5];if(!in.read((char*)hdr,sizeof hdr))throw runtime_error("header");
 if(hdr[0]!=0x4449534b)throw runtime_error("magic");if(hdr[1]<10||hdr[1]>14||hdr[2]!=15-hdr[1]||hdr[3]!=28-hdr[1]||!hdr[4]||hdr[4]>50000000)throw runtime_error("dimensions");
 B=(int)hdr[1];I=(int)hdr[2];F=(int)hdr[3];N=B+I;uint32_t nr=hdr[4];
 ifstream cf(argv[2]);int AA,CC,DD,K;cpp_int SS;if(!(cf>>AA>>CC>>DD>>SS>>K)||AA!=10306||CC!=21954||DD!=100000||SS!=cpp_int(1000000000000LL)||K<0||K>200000)throw runtime_error("Farkas header");
 map<vector<int>,bool> cert;
 for(int k=0;k<K;k++){
  int q;if(!(cf>>q)||q<0||q>B*B)throw runtime_error("signature size");vector<int> masks(q);
  for(int &m:masks)if(!(cf>>m)||m<=0||m>=(1<<B))throw runtime_error("signature mask");
  ...
  cpp_int cost=0;for(int i=0;i<B+q;i++)cost+=vals[i]*(i<B?AA:CC);if(cost>=DD*SS)throw runtime_error("Farkas strict margin");
 ...
 uint64_t group=2*B;for(int j=2;j<=I;j++)group*=j;cpp_int target(argv[3]),mass=0;Store store(nr);uint32_t ndirect=0,nfark=0,nres=0;int minstab=100000,maxstab=0;vector<int> sig;
 ...
 for(uint32_t idx=0;idx<nr;idx++){
  ...
  if(!store.insert(code))throw runtime_error("duplicate orbit");mass+=group/stab;minstab=min(minstab,stab);maxstab=max(maxstab,stab);
 ...
 ofstream report(prefix+".json");report<<"{\"verified\":true,\"B\":"<<B<<",\"I\":"<<I<<",\"representatives\":"<<nr<<",\"orbit_mass\":\""<<mass<<"\",\"group_size\":"<<group<<",\"min_stabilizer\":"<<minstab<<",\"max_stabilizer\":"<<maxstab<<",\"direct\":"<<ndirect<<",\"farkas\":"<<nfark<<",\"residual\":"<<nres<<",\"seconds\":"<<sec<<"}\n";
