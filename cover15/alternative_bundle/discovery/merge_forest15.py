...
R=Path(__file__).resolve().parent;rows=json.load(open(R/'metric15_residuals.json'));cand=json.load(open(R/'candidate15_map.json'))['residual_index'];counts=Counter();maxdepth=0;st=time.time();mar=None
head={'version':1,'residual_sha256':hashlib.sha256((R/'metric15_residuals.json').read_bytes()).hexdigest(),'radius_upper':str(Fraction('0.31814293085928')),'cases':len(rows)-1}
with gzip.open(R/'noncandidate15_trees.jsonl.gz','wt',compresslevel=6,encoding='utf8') as f:
 f.write(json.dumps(head,separators=(',',':'))+'\n')
 for k,row in enumerate(rows):
  if k==cand:continue
  with gzip.open(R/'forest15'/f'{k:05}.json.gz','rt',encoding='utf8') as g:c=json.load(g)
