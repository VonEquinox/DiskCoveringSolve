...
d=json.load(open(R/'active15_numeric.json'));z=[mp.mpf(str(x)) for x in d['x']]
t=time.time()
for k in range(8):
...
(R/'root15.json').write_text(json.dumps(out,separators=(',',':')))
P=[mp.matrix([mp.mpf(1),mp.mpf(0)])]+[mp.matrix(z[2*i:2*i+2]) for i in range(S.G//2)]
for f in S.FACES:
    a,b,c=[P[11+i] for i in f];A=mp.matrix([[2*(b-a)[i] for i in (0,1)],[2*(c-a)[i] for i in (0,1)]]);rhs=mp.matrix([(b.T*b)[0]-(a.T*a)[0],(c.T*c)[0]-(a.T*a)[0]])
