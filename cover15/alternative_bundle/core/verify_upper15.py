"""Exact upper cover for n=15: 39 triangles and 11 circular caps."""
...
def verify():
 ...
 assert len(S.FACES)==28-B and len(set(S.FACES))==28-B
 for face in S.FACES:
  if face in active_w:
 ...
 out={'verified':True,'vertices':B+N,'edges':len(ec),'faces':len(triangles),'boundary_caps':B,'exact_equality_center_faces':equality_faces,'strict_center_faces':slack,'minimum_orientation':str(minarea),'minimum_arc_wedge_margin':str(minarc),'oriented_triangles':oriented,'root_sha256':hashlib.sha256((R/'root15.json').read_bytes()).hexdigest(),'seconds':time.time()-st}
 (R/'upper15_verified.json').write_text(json.dumps(out,indent=2));print('UPPER COVER VERIFIED',len(triangles),'triangles,',equality_faces,'equality faces; min orientation',float(minarea),'sec',time.time()-st,flush=True)
 return out
