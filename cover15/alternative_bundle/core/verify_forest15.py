...
def verify():
 st=time.time();rows=json.load(open(ENUM/'metric15_residuals.json'));cand=check_mapping(rows)
 ...
 np=ENUM/'noncandidate15_trees.jsonl.gz';seen=set();counts=Counter();minmar=None;maxdepth=0;totalnodes=0
 with gzip.open(np,'rt',encoding='utf8') as f:
  head=json.loads(f.readline());assert head['version']==1 and head['residual_sha256']==sha(ENUM/'metric15_residuals.json') and head['radius_upper']==str(G.RU) and head['cases']==len(rows)-1
  for line in f:
 ...
 assert seen==set(range(len(rows)))-{cand} and len(seen)==43013
 assert md>0 and ml>0 and minmar>0
 out={'verified':True,'candidate':outcand,'noncandidate':{'cases':len(seen),'nodes':totalnodes,'counts':dict(counts),'minimum_force_margin':str(minmar),'minimum_force_margin_float':float(minmar),'maxdepth':maxdepth,'certificate_sha256':sha(np)},'root_sha256':sha(R/'root15.json'),'local_certificate_sha256':sha(R/'anchor_isolation_certificate.json'),'seconds':time.time()-st}
 (R/'forest15_verified.json').write_text(json.dumps(out,indent=2));print('ALL ANGLE TREES EXACTLY VERIFIED',out['noncandidate'],flush=True);return out
