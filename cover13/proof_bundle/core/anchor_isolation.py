...
GAMMA=148; LAMBDA=F(19,10000); NU=F(49,2500)
BUFFER=F(1,10**9); EPS=F(2,Q); RADIUS=F(1,8)

def h(p):return hashlib.sha256(p.read_bytes()).hexdigest()
def prepare():
 c=json.loads((ROOT/'cover13_krawczyk_cert.json').read_text())
 root=json.loads((ROOT/'cover13_kkt_root_119d.json').read_text())
 qx=int(c['Qx']); rho=F(int(c['rho_num']),int(c['rho_den']))
 x=[F(int(v),qx) for v in c['xnum']]
 def rnd(v):
  y=v*Q;return (2*y.numerator+y.denominator)//(2*y.denominator)
 a=[rnd(v) for v in x]
 for i in list(range(62))+list(range(63,119)):
  assert F(a[i],Q)-EPS<=x[i]-rho<=x[i]+rho<=F(a[i],Q)+EPS
 assert all(F(0)<x[i]-rho and x[i]+rho<F(1,10) for i in range(63,110))
 assert all(-NU<x[i]-rho and x[i]+rho<0 for i in range(110,119))
 coord=[[Q,0]]+[a[2*i:2*i+2] for i in range(31)]
 edges=[tuple(e) for e in root['edges']]
 assert len(edges)==47 and len(set(edges))==47
 A=[[0]*62 for _ in range(56)];M=[[0]*62 for _ in range(62)];degree=[0]*32
 ...
 for i in range(9):
  for d in range(2):
   A[47+i][2*i+d]=2*coord[i+1][d]
   M[2*i+d][2*i+d]+=a[110+i]
 # diagonal augmentation weights: w_e^2 on rods, NU^2 on circle equations.
 nuq=NU*Q;assert nuq.denominator==1
 di=[a[63+i]**2 for i in range(47)]+[nuq.numerator**2]*9
 H=[[M[i][j]*Q**3+GAMMA*sum(di[k]*A[k][i]*A[k][j] for k in range(56)) for j in range(62)] for i in range(62)]
 for i in range(62):
...
def main():
 ...
 assert cert['root_sha256']==h(ROOT/'cover13_krawczyk_cert.json')
 assert cert['root_data_sha256']==h(ROOT/'cover13_kkt_root_119d.json')
 ...
 out={'verified':True,'theorem':'Any realization of the 47-rod candidate graph with all rods <= r_star, unit anchors, fixed q0=(1,0), and ||Q-Q_star||_F <=1/8, is the candidate itself.',
 'gamma':GAMMA,'lambda':str(LAMBDA),'nu_upper':str(NU),'anchor_Frobenius_radius':str(RADIUS),
 'per_anchor_angle_halfwidth':str(F(1,24)),
