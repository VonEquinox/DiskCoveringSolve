...
using Code = std::array<uint64_t,5>; // C(13,3)=286

static int B,I,N,NF;
static std::vector<Tri> triples;
static int tid[13][13][13];
...
static bool code_less(const Code&a,const Code&b){
    // Compare as a 320-bit integer in a stable lexicographic order.
    for(int k=4;k>=0;--k){
...
struct GMap{std::array<int,13> p; std::vector<int> tmap;};
int main(int argc,char**argv){
    ...
    N=B+I;if(N!=13||B<9||B>12||I!=13-B||NF!=24-B||declared<=0)throw std::runtime_error("bad family/header");
