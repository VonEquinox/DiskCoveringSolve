...
from candidate_global17 import local
...
def check_mapping(rows):
 cm=json.load(open(ENUM/'candidate17_map.json'));k=cm['residual_index'];assert type(k)is int and 0<=k<len(rows) and rows[k]==cm['row'] and rows[k]['B']==S.B
...
def verify():
 (R/'forest17_verified.json').unlink(missing_ok=True)
 st=time.time();rows=json.load(open(ENUM/'metric17_residuals.json'));cand=check_mapping(rows)
 cp=R/'candidate17_tree.json.gz'
 ...
 np=ENUM/'noncandidate17_trees.jsonl.gz';seen=set();counts=Counter();minmar=None;maxdepth=0;totalnodes=0
 with gzip.open(np,'rt',encoding='utf8') as f:
  head=json.loads(f.readline());assert head['version']==1 and head['residual_sha256']==sha(ENUM/'metric17_residuals.json') and head['radius_upper']==str(G.RU) and head['cases']==len(rows)-1
  for line in f:
 ...
 out={'verified':True,'candidate':outcand,'noncandidate':{'cases':len(seen),'nodes':totalnodes,'counts':dict(counts),'minimum_force_margin':str(minmar),'minimum_force_margin_float':float(minmar),'maxdepth':maxdepth,'certificate_sha256':sha(np)},'root_sha256':sha(R/'root17.json'),'local_certificate_sha256':sha(R/'anchor_isolation_certificate.json'),'seconds':time.time()-st}
 (R/'forest17_verified.json').write_text(json.dumps(out,indent=2));print('ALL ANGLE TREES EXACTLY VERIFIED',out['noncandidate'],flush=True);return out
