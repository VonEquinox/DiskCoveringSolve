"""Rational interval operations and the certified n=17 root box."""
...
def rootbox(rho=F(1,10**80)):
 d=json.load(open(R/'root17.json'));q=int(d['Qx']);z=[F(int(v),q) for v in d['xnum']];iv=[(v-rho,v+rho) for v in z];p=[[point(1),point(0)]]+[iv[2*i:2*i+2] for i in range(S.G//2)]
 return d,z,iv,p
