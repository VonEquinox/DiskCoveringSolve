"""Exact upper cover for n=17: 44 triangles and 12 circular caps."""
from fractions import Fraction as F
from collections import Counter,defaultdict
from pathlib import Path
import json,time,hashlib
import system17 as S
import interval17 as I
...
def verify():
 (R/'upper17_verified.json').unlink(missing_ok=True)
 ...
 aa=json.load(open(R/'anchor_angles17.json'));cent=list(map(F,aa['centers']));h=F(aa['halfwidth'])
 assert len(cent)==B-1 and 0<cent[0]-h and cent[-1]+h<1 and h>0
 assert all(cent[i]+h<cent[i+1]-h for i in range(B-2))
 cuts=[(F(0),F(0))]+[(v-h,v+h) for v in cent]+[(F(1),F(1))]
 assert all(0<cuts[i+1][0]-cuts[i][1] and cuts[i+1][1]-cuts[i][0]<F(1,4) for i in range(B))
 ...
 out={'verified':True,'vertices':B+N,'edges':len(ec),'faces':len(triangles),'boundary_caps':B,'exact_equality_center_faces':equality_faces,'strict_center_faces':slack,'minimum_orientation':str(minarea),'minimum_arc_wedge_margin':str(minarc),'oriented_triangles':oriented,'root_sha256':hashlib.sha256((R/'root17.json').read_bytes()).hexdigest(),'seconds':time.time()-st}
 (R/'upper17_verified.json').write_text(json.dumps(out,indent=2));print('UPPER COVER VERIFIED',len(triangles),'triangles,',equality_faces,'equality faces; min orientation',float(minarea),'sec',time.time()-st,flush=True)
 return out
