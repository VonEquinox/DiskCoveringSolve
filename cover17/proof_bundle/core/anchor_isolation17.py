...
GAMMA=600; LAMBDA=F(17,10000); NU=F(11,1000)
BUFFER=F(1,10**8); EPS=F(2,Q); RADIUS=F(27,250)

def h(p):return hashlib.sha256(p.read_bytes()).hexdigest()
def prepare():
 c=json.loads((ROOT/'root17.json').read_text())
 ...
 assert error<BUFFER
 # Feasibility gives sum_e w_e |(A_rod d)_e| <= NU ||d_anchor||^2.
 # Consequently d^T A^T diag(w^2,NU^2) A d <=2 NU^2||d_anchor||^4.
 assert 2*GAMMA*NU**2*RADIUS**2<LAMBDA
 return H,error,c

def main():
 (ROOT/'anchor_isolation_verified.json').unlink(missing_ok=True)
 ...
 assert cert['root_sha256']==h(ROOT/'root17.json')
 assert cert['root_data_sha256']==h(ROOT/'system17.py')
 ...
 out={'verified':True,'theorem':'Any realization of the 63-rod candidate graph with all rods <= r_star, unit anchors, fixed q0=(1,0), and ||Q-Q_star||_F <=27/250, is the candidate itself.',
