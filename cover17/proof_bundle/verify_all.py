#!/usr/bin/env python3
"""Fail-closed, fresh, standard-library accepting pipeline for the n=17 theorem."""
...
def main():
    ...
    required=['PROOF_zh.md','README.md','core/system17.py','core/root17.json',
              'enumeration/peel_enum.cpp','enumeration/peel_audit.cpp',
              'core/candidate17_tree.json.gz','enumeration/noncandidate17_trees.jsonl.gz']
    assert all(x in before for x in required),'missing proof input'
    sequence=['verify_interfaces17.py','verify_root17.py','verify_upper17.py',
              'anchor_isolation17.py','verify_enumeration17.py','verify_forest17.py','test_fail_closed17.py']
    ...
    root=read('root17_verified.json');upper=read('upper17_verified.json')
    local=read('anchor_isolation_verified.json');interfaces=read('interfaces17_verified.json')
    enumeration=read('enumeration17_verified.json');forest=read('forest17_verified.json');tests=read('negative17_verified.json')
    assert all(x['verified'] is True for x in (root,upper,local,interfaces,enumeration,forest,tests))
    rh=before['core/root17.json']
    assert all(x['root_sha256']==rh for x in (root,upper,local,interfaces,forest))
    assert enumeration['residual_sha256']==before['enumeration/metric17_residuals.json']
    assert forest['local_certificate_sha256']==before['core/anchor_isolation_certificate.json']
    assert forest['candidate']['certificate_sha256']==before['core/candidate17_tree.json.gz']
    assert forest['noncandidate']['certificate_sha256']==before['enumeration/noncandidate17_trees.jsonl.gz']
    ...
    out={'verified':True,'theorem':'r_17 = sqrt(t_star), R_17 = 1/sqrt(t_star)',
    ...
    print('R17 COMPLETE CERTIFICATE CHAIN VERIFIED',flush=True)
