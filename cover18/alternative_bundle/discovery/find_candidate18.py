...
import system18 as S,json,itertools
rows=json.load(open(S.R/'metric18_residuals.json'));dic={tuple(sorted(tuple(f) for f in row['faces'])):k for k,row in enumerate(rows) if row['B']==S.B};matches=[]
...
(S.R/'candidate18_map.json').write_text(json.dumps(out,indent=2))
