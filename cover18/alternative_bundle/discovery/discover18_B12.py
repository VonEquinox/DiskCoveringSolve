...
if __name__=='__main__':
    start=time.time();rng=np.random.default_rng(1821)
    best=1.;path=pathlib.Path(__file__).parent
    for k in range(int(sys.argv[1]) if len(sys.argv)>1 else 30):
        th=np.arange(B)*2*np.pi/B
        if k%3==0:
            ins=np.r_[np.zeros((1,2)),.45*np.c_[np.cos(np.arange(5)*2*np.pi/5+.15*(k//3)),np.sin(np.arange(5)*2*np.pi/5+.15*(k//3))]]
        elif k%3==1:
            th2=np.arange(6)*2*np.pi/6+.13*(k//3)
            ins=.38*np.c_[np.cos(th2),np.sin(th2)]
        else:
            ins=np.array([[-.36,-.22],[.0,-.28],[.36,-.22],[-.36,.22],[.0,.28],[.36,.22]])
        P=np.r_[.84*np.c_[np.cos(th),np.sin(th)],ins]
        P+=rng.normal(0,.003 if k<10 else .04,P.shape)
        try:fa,E=data(P);d=solve(P,fa,E)
        except Exception as ex:print(k,ex,flush=True);continue
        print(k,d['r'],d['iterations'],d['success'],'sec',time.time()-start,flush=True)
        (path/f'trial18_{k:03}.json').write_text(json.dumps(d,indent=2))
        if d['t']<best and min(d['rod_slacks'])>-1e-9:
            best=d['t'];(path/'candidate18_B12_numeric.json').write_text(json.dumps(d,indent=2))
