...
def verify():
 st=time.time();rows=json.load(open(ENUM/'survivors18.json'));cand=check_mapping(rows)
 ...
 with gzip.open(np,'rt',encoding='utf8') as f:
  head=json.loads(f.readline());assert head['version']==1 and head['residual_sha256']==sha(ENUM/'survivors18.json') and head['radius_upper']==str(G.RU) and head['cases']==len(rows)-1
  for line in f:
