...
class RunnerTests(unittest.TestCase):
    def setUp(self):
        ...
        temp = tempfile.TemporaryDirectory(prefix="cover18_runner_test_")
    ...
    def fake_run(self, command, *, cwd, check, env):
        ...
        result = {"verified": True, "unresolved_leaves": 0, "kkt_dimension": 174,
                  orbit_key: 16220, "noncandidate_forest": {"cases": 16219},
                  "steps": [{key: s} for s in runner.STEPS[self.variant]],
                  "negative_tests": {"tests_passed": 32, "rejection_tests": 32},
                  "small_unpruned_count_tests": 26, "unpruned_recurrence_tests": {"cases": 29}}
        if self.corrupt:
            self.corrupt(result)
    ...
    def test_proof_mismatch(self):
        (self.root / "docs/r18_alternative_proof_zh.md").write_text("modified")
    ...
    def test_missing_tests(self):
        self.corrupt = lambda result: result.update(small_unpruned_count_tests=0)
        with patch.object(runner.subprocess, "run", side_effect=self.fake_run):
