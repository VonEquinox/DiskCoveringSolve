/* Independent root-face enumeration and integer angular pruning, n=20.
...
#include <vector>
using namespace std;
constexpr int MAXV=20,MAXT=27;
constexpr long long UNIT=10000000, ARC=873043, TWO=1822179, THREE=3019184;
...
void enumerate(Board const&s,vector<Region> const&tasks){
 ...
 size_t choice=0; for(size_t j=1;j<tasks.size();j++){ if(tasks[j].budget<tasks[choice].budget || (tasks[j].budget==tasks[choice].budget && tasks[j].rim.size()>tasks[choice].rim.size())) choice=j; }
 auto current=tasks[choice]; vector<Region> tail=tasks; tail.erase(tail.begin()+choice);
