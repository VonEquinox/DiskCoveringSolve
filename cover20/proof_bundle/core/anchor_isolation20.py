...
GAMMA=200; LAMBDA=F(11,25000); NU=F(9,1000)
BUFFER=F(1,10**8); EPS=F(2,Q); RADIUS=F(11,100)

def h(p):return hashlib.sha256(p.read_bytes()).hexdigest()
def prepare():
 c=json.loads((ROOT/'root20.json').read_text())
...
def main():
 ...
 assert cert['root_sha256']==h(ROOT/'root20.json')
 assert cert['root_data_sha256']==h(ROOT/'system20.py')
 ...
 out={'verified':True,'theorem':'Any realization of the 83-rod candidate graph with all rods <= r_star, unit anchors, fixed q0=(1,0), and ||Q-Q_star||_F <=11/100, is the candidate itself.',
