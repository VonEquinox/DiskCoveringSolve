...
def verify():
 ...
 assert all(v-F(1,10**80)>0 for v in z[S.P:S.P+S.M]);assert all(-F(9,1000)<v-F(1,10**80) and v+F(1,10**80)<0 for v in z[S.P+S.M:])
 ...
 out={'verified':True,'dimension':n,'boxes':reports,'minimum_weight':str(min(z[S.P:S.P+S.M])-F(1,10**80)),'r_interval':[str(rlo),str(rhi)],'R_interval':[str(Rlo),str(Rhi)],'decimal_intervals':decimals,'root_sha256':hashlib.sha256((R/'root20.json').read_bytes()).hexdigest(),'seconds':time.time()-st}
 (R/'root20_verified.json').write_text(json.dumps(out,indent=2));print('ROOT VERIFIED',n,'ratio',float(F(reports[1]['inclusion_ratio'])),'r',float(rlo),'sec',time.time()-st,flush=True)
 return out
