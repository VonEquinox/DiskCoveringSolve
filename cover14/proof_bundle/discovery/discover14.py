...
if __name__=='__main__':
    start=time.time();rng=np.random.default_rng(14)
    best=1.;path=pathlib.Path(__file__).parent
    for k in range(int(sys.argv[1]) if len(sys.argv)>1 else 10):
        th=np.arange(B)*2*np.pi/B
        P=np.r_[.82*np.c_[np.cos(th),np.sin(th)],.33*np.c_[np.cos(np.arange(4)*np.pi/2+.2*(k%5)),np.sin(np.arange(4)*np.pi/2+.2*(k%5))]]
        ...
        if d['t']<best and min(d['rod_slacks'])>-1e-9:
            best=d['t'];(path/'candidate14_numeric.json').write_text(json.dumps(d,indent=2))
