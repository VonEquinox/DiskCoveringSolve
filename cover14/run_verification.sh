...
# Homebrew installs Boost as keg-only on some macOS versions. Expose its
# headers without changing the submitted verifier.
...
"$PYTHON_BIN" -S -B "$ROOT/verify_manifest.py"
cd "$BUNDLE"
"$PYTHON_BIN" -S -B verify_all.py

echo "COVER14 EXACT MASTER REPLAY PASSED"
