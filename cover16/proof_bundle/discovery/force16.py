...
BMIN=11; N=16; V=46;QL=10**15;DF=10**36
A=9974;C=21143;C6=37566;D=100000;RU=F('0.30821980189863')

def edges(B,faces):
 assert len(faces)==30-B
 return [(i,B+i) for i in range(B)]+[(i,B+(i+1)%B) for i in range(B)]+[(B+16+k,B+c) for k,fa in enumerate(faces) for c in fa]
...
if __name__=='__main__':
 rr=json.load(open(R/'metric16_residuals.json'));st=time.time()
