"""Exact upper cover for n=16: 42 triangles and 12 circular caps."""
from fractions import Fraction as F
from collections import Counter,defaultdict
from pathlib import Path
import json,time,hashlib
import system16 as S
import interval16 as I
...
def verify():
 ...
 aa=json.load(open(R/'anchor_angles16.json'));cent=list(map(F,aa['centers']));h=F(aa['halfwidth'])
 ...
 out={'verified':True,'vertices':B+N,'edges':len(ec),'faces':len(triangles),'boundary_caps':B,'exact_equality_center_faces':equality_faces,'strict_center_faces':slack,'minimum_orientation':str(minarea),'minimum_arc_wedge_margin':str(minarc),'oriented_triangles':oriented,'root_sha256':hashlib.sha256((R/'root16.json').read_bytes()).hexdigest(),'seconds':time.time()-st}
 (R/'upper16_verified.json').write_text(json.dumps(out,indent=2));print('UPPER COVER VERIFIED',len(triangles),'triangles,',equality_faces,'equality faces; min orientation',float(minarea),'sec',time.time()-st,flush=True)
 return out
