...
def check_mapping(rows):
 cm=json.load(open(ENUM/'candidate16_map.json'));k=cm['residual_index'];assert type(k)is int and 0<=k<len(rows) and rows[k]==cm['row'] and rows[k]['B']==S.B
...
def verify():
 st=time.time();rows=json.load(open(ENUM/'metric16_residuals.json'));cand=check_mapping(rows)
 cp=R/'candidate16_tree.json.gz'
 ...
 np=ENUM/'noncandidate16_trees.jsonl.gz';seen=set();counts=Counter();minmar=None;maxdepth=0;totalnodes=0
 with gzip.open(np,'rt',encoding='utf8') as f:
  head=json.loads(f.readline());assert head['version']==1 and head['residual_sha256']==sha(ENUM/'metric16_residuals.json') and head['radius_upper']==str(G.RU) and head['cases']==len(rows)-1
  for line in f:
 ...
 out={'verified':True,'candidate':outcand,'noncandidate':{'cases':len(seen),'nodes':totalnodes,'counts':dict(counts),'minimum_force_margin':str(minmar),'minimum_force_margin_float':float(minmar),'maxdepth':maxdepth,'certificate_sha256':sha(np)},'root_sha256':sha(R/'root16.json'),'local_certificate_sha256':sha(R/'anchor_isolation_certificate.json'),'seconds':time.time()-st}
 (R/'forest16_verified.json').write_text(json.dumps(out,indent=2));print('ALL ANGLE TREES EXACTLY VERIFIED',out['noncandidate'],flush=True);return out
