...
def main():
 t0=time.time();runs=[];logdir=VERIFY/'master_logs';logdir.mkdir(parents=True,exist_ok=True)
 ...
 for name,cmd,report in MODULES:
  ...
  runs.append({'name':name,'seconds':time.time()-st,'report':report.name,'log':str(log.relative_to(BASE))})
 ...
 (VERIFY/'final_verification.json').write_text(json.dumps(out,indent=2))
